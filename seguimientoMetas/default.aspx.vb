...
#Region " Web Form Designer Generated Code "
    ...
    Protected WithEvents ddlAnoTLM As System.Web.UI.WebControls.DropDownList
    Protected WithEvents ddlMesTLM As System.Web.UI.WebControls.DropDownList
    Protected WithEvents ddlTLM As System.Web.UI.WebControls.DropDownList
    Protected WithEvents bBuscarPorTLM As System.Web.UI.WebControls.Button
    Protected WithEvents Panel5 As System.Web.UI.WebControls.Panel
    Protected WithEvents ddlSalesAdv As System.Web.UI.WebControls.DropDownList
    Protected WithEvents ddlAnoSalesAdv As System.Web.UI.WebControls.DropDownList
    Protected WithEvents ddlMessalesAdv As System.Web.UI.WebControls.DropDownList
    Protected WithEvents bBuscarPorSalesAdv As System.Web.UI.WebControls.Button
...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Page.IsPostBack = False Then
            ...
            '-------------------------------------------------------
            'Cargamos codigos de agentes KAM/Telemarketing
            '-------------------------------------------------------
            With ddlTLM
                '.DataSource = listaEjecutivosComercialesPorCelula(codigoSociedad, codigoCelula)
                .DataSource = ws.listaPromotoras("GMSC")
                .DataTextField = "nom_promotora"
                .DataValueField = "cod_promotora"
                .DataBind()
            ...
            '-------------------------------------------------------
            'Cargamos codigos de agentes ventas telefono
            '-------------------------------------------------------
            With ddlVETEL
            ...
            '-------------------------------------------------------
            'Cargamos codigos de agentes Sales Advisor
            '-------------------------------------------------------
            With ddlSalesAdv
                .DataSource = listaVendedoras("GMSC", "EJEVE", "VTE")
            ...
            Else
                If (grupoAgente = "CELULA") Or (grupoAgente = "TELEMKTG") Then
                    ...
                    ''''    End While
                    If tipoAgente = "EJEVE" Then
                        i = 0
                        While i <= ddlSalesAdv.Items.Count - 1
                            If ddlSalesAdv.Items(i).Value <> codigoAgente Then
                                ddlSalesAdv.Items.RemoveAt(i)
                            Else
                                i += 1
                            End If
                        End While

                        If Not IsNothing(ddlSalesAdv.Items.FindByValue(codigoAgente)) Then
                            ddlSalesAdv.Items.FindByValue(codigoAgente).Selected = True
                        End If
                    Else
                        i = 0
                        While i <= ddlTLM.Items.Count - 1
                            If ddlTLM.Items(i).Value <> codigoAgente Then
                                ddlTLM.Items.RemoveAt(i)
                            Else
                                i += 1
                            End If
                        End While

                        If Not IsNothing(ddlTLM.Items.FindByValue(codigoAgente)) Then
                            ddlTLM.Items.FindByValue(codigoAgente).Selected = True
                        End If
                    End If
                ElseIf grupoAgente = "EJEVE" Then
                    '------------------------------------------------
                    'Dejamos sólo al sales advisor que corresponda
                    '------------------------------------------------
                    i = 0
                    While i <= ddlSalesAdv.Items.Count - 1
                        If ddlSalesAdv.Items(i).Value <> codigoAgente Then
                ...
                ElseIf grupoAgente = "VETEL" Then
                    ...
                    While i <= ddlVETEL.Items.Count - 1
                        If (codigoAgente <> "010" And codigoAgente <> "048" And ddlVETEL.Items(i).Value <> codigoAgente) Or _
                            ((codigoAgente = "010" Or codigoAgente = "048") And (ddlVETEL.Items(i).Value <> "1048")) Then
                    ...
                    If codigoAgente = "010" Or codigoAgente = "048" Then
                        If Not IsNothing(ddlVETEL.Items.FindByValue("1048")) Then
                            ddlVETEL.Items.FindByValue("1048").Selected = True
                        End If
                    End If
            ...
            For i = 2010 To Now.Year
                ...
                ddlAnoTLM.Items.Add(New ListItem(i, i))
                ddlAnoVETEL.Items.Add(New ListItem(i, i))
                ddlAnoVTPUB.Items.Add(New ListItem(i, i))
                ddlAnoSalesAdv.Items.Add(New ListItem(i, i))
            Next
        ...
        Else
            ...
            Select Case grupoAgente
                Case "CELULA", "TELEMKTG"
                    ...
                    If tipoAgente = "EJEVE" Then
                        'ocultamos solapa KAM/TELEMARKETING
                        Literal1.Text &= "$('#op5').hide();" & vbCrLf
                        Literal1.Text &= "$('#tab5').hide();" & vbCrLf

                        'Dejamos activa la solapa SALES ADVISOR
                        Literal1.Text &= "$('#op6').addClass('active').show();" & vbCrLf & _
                                        "$('#tab6').show();"
                    Else
                        'ocultamos solapa SALES ADVISOR
                        Literal1.Text &= "$('#op6').hide();" & vbCrLf
                        Literal1.Text &= "$('#tab6').hide();" & vbCrLf

                        'Dejamos activa la solapa KAM/TELEMARKETING
                ...
                Case "VTPUB"
                    ...
                    'ocultamos solapa TELEMARKETING
                    Literal1.Text &= "$('#op5').hide();" & vbCrLf
                    Literal1.Text &= "$('#tab5').hide();" & vbCrLf
    ...
    'Private Sub bBuscarPorCelula_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
    '    inicializaPaneles()
    ...
    'Private Sub bBuscarPorAgente_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
    '    inicializaPaneles()
    ...
    Private Sub bBuscarPorAgenteTLM_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bBuscarPorTLM.Click
        inicializaPaneles()

        Dim tituloTabla As String = "Agente: " & ddlTLM.SelectedItem.Text.Trim
        Dim codigoAgente As String = ddlTLM.SelectedValue
        Dim anoPeriodo As Integer = ddlAnoTLM.SelectedValue
        Dim mesPeriodo As Integer = ddlMesTLM.SelectedValue

        'Vamos a buscar datos y los guardamos en variable de sesión
        Dim dt As DataTable = obtieneInfoMetasPorEjecutivo(codigoSociedad, codigoAgente, anoPeriodo, mesPeriodo)
        ...
        Panel5.Controls.Add(generaTablaResultados(dt, _
                                                ...
                                                codigoAgente, fechaConsulta))
        Panel3.Visible = True

        Literal1.Text &= "$('ul.tabs li').removeClass('active'); " & vbCrLf & _
                        "$('.tab_content').hide();" & vbCrLf & _
                        "$('#op5').addClass('active').show();" & vbCrLf & _
                        "$('#tab5').show();"
    End Sub

    Private Sub bBuscarPorSalesAdv_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bBuscarPorSalesAdv.Click
        inicializaPaneles()

        Dim tituloTabla As String = "Agente: " & ddlSalesAdv.SelectedItem.Text.Trim
        Dim codigoAgente As String = ddlSalesAdv.SelectedValue
        Dim anoPeriodo As Integer = ddlAnoSalesAdv.SelectedValue
        Dim mesPeriodo As Integer = ddlMessalesAdv.SelectedValue
        ...
        Literal1.Text &= "$('ul.tabs li').removeClass('active'); " & vbCrLf & _
                        "$('.tab_content').hide();" & vbCrLf & _
                        "$('#op6').addClass('active').show();" & vbCrLf & _
                        "$('#tab6').show();"
    End Sub
...
#Region " Funciones de búsqueda "
    ...
    Private Function obtieneInfoMetasPorCelulaPorAgente(ByVal codigoSociedad As String, _
                                             ByVal codigocelula As String, _
                                             ByVal anoPeriodo As Integer, _
                                            ByVal mesPeriodo As Integer, _
                                            ByVal username As String) As DataTable

        Const spName = "ido_sel_info_metas_x_celula_x_usuario"
        Dim dbConn As New SqlConnection
        Dim spCall As New SqlCommand(spName, dbConn)
        Dim daSql As New SqlDataAdapter
        Dim resultDT As New DataTable
